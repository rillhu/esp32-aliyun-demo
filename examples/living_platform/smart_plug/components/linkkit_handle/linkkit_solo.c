...
{
    int res = 0;
    cJSON *root = NULL, *PlugSwitch = NULL, *LightColor = NULL;
    ...
    PlugSwitch = cJSON_GetObjectItem(root, "PowerSwitch");
    if (PlugSwitch) {
        //lightbulb_set_on(PlugSwitch->valueint);
        
        ESP_LOGI(TAG,"Plug will action with PowerSwitch %d",PlugSwitch->valueint);
        if(PlugSwitch->valueint){
            plug_on();
        }else{
            plug_off();
        }
    ...
    //PlugSwitch = cJSON_GetObjectItem(root, "RGBColor");
    //if (PlugSwitch) {
    
        //LightColor = cJSON_GetObjectItem(PlugSwitch, "Red");
        //lightbulb_set_hue(LightColor ? LightColor->valueint : 0);
        //LightColor = cJSON_GetObjectItem(PlugSwitch, "Green");
        //lightbulb_set_hue(LightColor ? LightColor->valueint : 120);
        //LightColor = cJSON_GetObjectItem(PlugSwitch, "Blue");
